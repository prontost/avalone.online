...
> Краткая выжимка. Глубокий анализ с архитектурными trade-offs, эволюцией по фазам и деталями каждого слоя — в [`docs/tech-stack.md`](docs/tech-stack.md).

### Базовый выбор

- **Frontend:** SvelteKit PWA.
- **Backend:** Python FastAPI.
- **Architecture:** Modular monolith с извлекаемыми сервисами.
- **Database:** PostgreSQL (start) → Neo4j (graph at scale).
- **Cache / Pub-Sub:** Redis.
- **Real-time:** WebSockets + Redis Pub/Sub.
- **Auth:** Firebase Auth → DID/VC roadmap.
- **Maps:** Kakao Maps API.
- **Payments:** Stripe → KakaoPay / Toss Payments.
- **ML:** Python + scikit-learn + pgvector.
- **Hosting:** MacBook + Cloudflare tunnel → VPS + Docker → Kubernetes.
